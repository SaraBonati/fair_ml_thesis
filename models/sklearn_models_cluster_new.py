...
class Model:
    ...
    def __init__(self, train_data_source, test_data_paths, task, columns, cat_columns, num_columns,
        ...
        self.clf_set = {'LinearSVC': LinearSVC(C=1, max_iter=10000, random_state=self.seed),
                        'LogReg': LogisticRegression(max_iter=10000, random_state=self.seed),
    ...
    def test_model_spatial(self):
        ...
        for clf_name, clfier in self.clf_set.items():
            ...
            for t in range(len(self.test_data_paths)):
                ...
                if clf_name in ["LogReg", "LinearSVC", "XGBoost"]:
        ...
        for clf_name, clfier in self.clf_set.items():
            ...
            for t in range(len(self.test_data_paths)):
                ...
                metricss[clf_name][self.test_years[t]]['rac_dpr'] = demographic_parity_ratio(y_test,
                                                                                             ...
                                                                                             sensitive_features=race_test,
                                                                                             method="between_groups")
                metricss[clf_name][self.test_years[t]]['rac_edd'] = equalized_odds_difference(y_test,
                                                                                              y_pred,
                                                                                              sensitive_features=race_test,
                                                                                              method="between_groups")
                ...
                if clf_name in ["LogReg", "LinearSVC", "XGBoost"]:
                    ...
                    if not os.path.isdir(
                            os.path.join(rdir, self.task, str(self.train_year), 'sklearn', self.train_state)):
                        os.makedirs(os.path.join(rdir, self.task, str(self.train_year), 'sklearn', self.train_state))
                    # save to pickle file
                    with open(os.path.join(rdir, self.task, str(self.train_year), 'sklearn', self.train_state,
                                           f'2temporal_{self.train_state}_test_{self.test_states[t]}'
                                           f'_{clf_name}.pickle'),
                              'wb') as handle:
                        pickle.dump(ROC_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
if __name__ == "__main__":
    ...
    rdir = os.path.join(os.path.split(wdir)[0], "fair_ml_thesis_data", "results2")
    ...
    if args.mode == "spatial":
        ...
        for select_year in [args.year]:
            data_file_paths = glob.glob(
                os.path.join(ddir, str(select_year), '1-Year') + f'/{str(select_year)}_*_{task}.csv')
    ...
    if args.mode == "temporal":
        ...
        for select_year in [args.year]:
            ...
            test_data_file_paths = [
                os.path.join(ddir, str(y), '1-Year', f'{str(y)}_{args.train}_{task}.csv')
